...
const syntaxLabel = "ES2019 / Node.js 12";
const items = ref([
...
const ranked = computed(() => sortDesc(items.value.map(function (i) { return i.score != null ? i.score : 0; })));

function bumpScores() {
  items.value = items.value.map((item) => ({
    ...item,
    score: Math.min(100, (item.score != null ? item.score : 0) + 1),
  }));
}
</script>
